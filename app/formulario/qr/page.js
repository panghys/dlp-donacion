...
import QRCode from "react-qr-code"; // Librería para generar QR
import { useRef } from "react";

export default function QRPage() {
  const searchParams = useSearchParams(); // Hook para capturar los parámetros de la URL
  const id = searchParams.get("id"); // Extrae el parámetro "id" de la URL

  const linkqr = `https://dlp-prestamo.vercel.app/PDRL?libro=${id}`;
  const qrRef = useRef(); // Referencia para manipular el QR
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen bg-custom-gradient">
      ...
      <p className="text-customBlue font-sans text-center opacity-60 mb-8">
        A continuación, descargue e imprima el código QR y péguelo sobre su libro.
      </p>
